...
many of which correspond to the known open clusters, and stars in the OB associations.
...
\begin{figure*}[htbp]
  \begin{center}
    \includegraphics[width=\textwidth]{figures/a_vorb.pdf}
  \end{center}
  \caption{%
    Binary orbital velocity as a function of semi-major axis for $2~M_\odot$
    binary.
    \label{fig:vorbit}}
\end{figure*}
...
velocity (Figure~\ref{fig:vorbit}).
...
\begin{figure*}[htbp]
  ...
  \caption{%
    ...
    Each star (node) is marked as red circles, and a line (edge) is drawn
...
\begin{figure*}[htbp]
  ...
  \caption{%
    ...
    Pair networks of sizes less 6 are connected by gray lines.
    Pair networks of size larger than 6 are plotted with a unique color per network.
...
\begin{figure*}[htbp]
  ...
  \caption{
    ...
    Figure~\ref{fig:graphviz_examples}. Each panel corresponds to the panel
    at the same position in Figure~\ref{fig:graphviz_examples}.
