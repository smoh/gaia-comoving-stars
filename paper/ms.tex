...
using the \gaia\ data archive (\todo{cite}).
For each star, we also include the positional measurements from \tgas,
\gaia\ $G$-band and \tmass\ $J$, $H$-band magnitudes as well as
...
star graph is in the group table. We assign a unique index to each group
...
Additionally, given the strong correlation between the larger connected 
components of co-moving pairs and the known star clusters,
we provided
%
\begin{itemize}
  \item index and name of the closest cluster in the Milky Way Star Cluster catalog of
...
% The distances and physical separations are calculated using the point-estimate
% of distances (\eqname~\ref{eq:dist}).
