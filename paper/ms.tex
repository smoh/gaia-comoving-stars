...
\title{Co-moving stars in \textsl{Gaia DR1}:\\
  Abundance of very wide separation co-moving pairs}
...
Some of these pairs form a larger group through mutual co-moving neighbors,
many of which correspond to the known open clusters, and stars in OB associations.
However, we also discover many co-moving groups of pairs for the first time.
More surprisingly, we find a large number
of very wide ($>3$~pc) separation co-moving pairs of stars.
The number of these very wide separation pairs only increase with increasing
separation, and cannot be merely due to false positives.
We present a catalog of high-confidence co-moving pairs of stars in the \tgas\
with auxiliary information, and discuss its potential usage
...
\begin{figure*}[htbp]
  ...
  \caption{%
    ...
    clusters, but we also discover quite a few for the first time.
...
However, there are still many larger connected components that we
