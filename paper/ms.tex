...
\begin{eqnarray}
  ...
  \quad ,
\end{eqnarray}
where the factor of $r_i^2\,r_j^2$ is the Jacobian from $\vec{y}$
to data ($\transp{(\mu_{\alpha,i}\ \,\mu_{\delta,i}\ \,\mu_{\alpha,j}\ \,\mu_{\delta,j})}$) space.
...
\end{eqnarray}
We can identify $\mat{A}$, $\mat{B}\,\vec{b}$, and $C$ in
$\ln p(\vec{v}) + \ln p(\data \given \vec{v}, r_i, r_j)$, and write down
$\vec{\nu}$ and $\Delta$ using \eqname~\ref{eq:convnu} and \ref{eq:convDelta} as
