...
separation; and cannot be explained purely by false-positives contamination.
...
\begin{itemize}
  ...
  \item We assume that the 3D velocities of stars in a given pair
    ...
    the velocity prior $p(\vec{v})$ that we describe later.
\end{itemize}

Under these assumptions, the likelihood of a proper motion measurement,
$\bs{\mu}$, for a star with true distance, $d$, and true tangential velocity,
$\vec{v}_t =
(\begin{array}[t]{c c} v_\alpha & v_\delta\end{array})^\mathsf{T}$ is
\begin{align}
  L(\vec{\mu} \given \vec{v}, d) &=
...
\end{align}
where the tangential velocity $\vec{v}_t$ is related to the 3D velocity
...
L_j$, are marginalized over the (unknown) true 3D velocity and distance for each star in
the pair $(i,j)$.
\begin{align}
  \mathcal{L}_1 &=
...
\begin{equation}
  Q_i = \int \, \dd d \, \dd^3 \vec{v} \, L(\vec{v}, d, 0) \, p(\vec{v}) \, p(d\given\varpi_i)
\end{equation}
...
To account for this, we modify set $s^2 = \frac{2 \, G \, \msun}{|\vec{x}_i-\vec{x}_j|}$.
...
marginalize over the (unknown) three components of the velocities 
in contrast to just working in the proper motion space.
