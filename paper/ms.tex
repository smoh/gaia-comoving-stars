...
\textsl{Tycho-Gaia Astrometric Solution} (\tgas). The precision and size of this sample
presents an opportunity to find new binary stars and moving groups, especially
rare binaries or sparsely populated groups.
Here, we look for high-confidence co-moving pairs of stars using this data.
We use a justified marginalized likelihood ratio test to separate
pairs of stars with similar 3D velocities from
those consistent with being drawn independently from the field
population.  Although we perform some visualizations using
(bias-corrected) inverse parallax as point-estimate of distance,
the likelihood ratio test
is based on a probabilistic model that includes the covariances
of parallax and proper motions, and marginalizes the unknown
distances and 3D velocities of stars.
We find 13,085 pairs of co-moving stars among 10,606 unique stars
out to very wide separations up to 10~pc, the limit of our search.
Some of these pairs form larger groups through mutual co-moving neighbors,
many of which correspond to the known open clusters, and OB associations.
We also report many new co-moving groups of pairs.
More surprisingly, we find a large number
of very wide ($>3$~pc) separation co-moving pairs of stars.
The number of these very wide separation pairs increase with increasing
separation; and cannot be explained purely by false-positive contamination.
We present a catalog of high-confidence co-moving pairs of stars in the \tgas\
with auxiliary information, and discuss its potential use
for making dynamical inferences, and testing stellar models and chemical
...
in tangential velocity less than $\absdvtan < 10$~\kms.
...
For wide binaries, the assumption of the stars having the same 3D velocity for 
hypothesis (1) can
...
$p(\vec{v})$, it has minimal effect on the hypothesis (2).
...
This is primarily because of the non-uniform coverage of the \tgas\ data.
...
that such large scale phase-space correlations may now be looked for and studied using the \gaia\ data.
...
3D velocities, incorporating the covariances of parallax and proper motions.
...
by a path.
The entire 13,058 candidate co-moving pairs are grouped into 4555 connected
...
If so, this population should still be relatively young compared
...
in a recent work by \citet{2016arXiv161107883O}. They find $\approx 1900$ wide binaries
...
marginalize over the (unknown) distances and velocities of the stars
