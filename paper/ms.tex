...
pairs of stars with similar three-space velocities from
...
However, there still is a surprisingly large number
of very wide ($>3$~pc) separation pairs of stars that are co-moving.
We present a catalog of high-confidence co-moving pairs of stars in \tgas\
with auxiliary information,
and discuss its potential usage
...
\begin{itemize}
  \item At small separations ($<1$~pc), there is a population of pairs with
    ...
    pairs are very close in both position and proper motion, it is highly
...
\begin{figure*}[htbp]
  ...
  \caption{%
    ...
    On the top row, we show how the distribution of pairs on \absdvtan\ vs. separation changes
    in slices of $\ln \mathcal{L}_1 /\mathcal{L}_2$ (sequentially color-coded in blue).
    On the bottom row, we show how the distribution of $\ln \mathcal{L}_1 /\mathcal{L}_2$
    changes in slices of \absdvtan\ (sequentially color-coded in red).
